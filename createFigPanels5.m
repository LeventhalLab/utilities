...
%       .leftMargin = space left at the left of the figure for y-axis
%           labels
%
% VARARGs:
...
fullPanelWidth = sum(figProps.panelWidth) + sum(figProps.colSpacing) + figProps.leftMargin;
...
if fullPanelWidth > figProps.width
    disp('panels will not fit horizontally');
    return;
end
...
for iRow = 1 : m
    ...
    for iCol = 1 : n
        ...
        leftEdge = figProps.leftMargin + sum(figProps.panelWidth(1:iCol-1)) + sum(figProps.colSpacing(1:iCol-1));
        if iRow == m
            botEdge = botMargin;
        else
